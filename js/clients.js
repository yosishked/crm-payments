...
var Clients = (function() {
  ...
  function _priceRowEditable(label, amount, canEdit, onclickAction) {
    var cls = amount < 0 ? ' class="price-negative"' : '';
    var editBtn = canEdit
      ? ' <button class="btn-icon btn-icon-sm" onclick="' + onclickAction + '" title="' + UI.escapeHtml('ערוך') + '">&#9998;</button>'
      : '';
    return '<div class="price-row">' +
      ...
      for (var i = 0; i < transactions.length; i++) {
        ...
        html += '<tr>' +
          ...
          '<td>' + (typeof isAdmin === 'function' && isAdmin() ?
            '<button class="btn-icon" onclick="Clients.editPayment(\'' + UI.escapeHtml(tx.id) + '\', \'' + lid + '\')" title="' + UI.escapeHtml('ערוך') + '">&#9998;</button>' +
            '<button class="btn-icon btn-icon-danger" onclick="Clients.deletePayment(\'' + UI.escapeHtml(tx.id) + '\', \'' + lid + '\')" title="' + UI.escapeHtml('מחק') + '">&#128465;</button>' : '') + '</td>' +
        '</tr>';
      }
